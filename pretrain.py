...
from model import WindowedModel, MultiOutputPopulationNet
from utils import AverageMeter, ProgressMeter, get_device, save_checkpoint


parser = ArgumentParser(description='Genome Synthesis Pretraining')
...
parser.add_argument('--model-name', type=str, default='pretrain',
...
def main() -> None:
    ...
    kwargs = {'model_class': MultiOutputPopulationNet, 'total_size': vcf_reader.positions.shape[0], 'window_size': args.window_size, 'num_layers': args.layers, 'num_classes': len(vcf_reader.label_encoder.classes_), 'num_super_classes': len(vcf_reader.super_label_encoder.classes_)}
    model = WindowedModel(**kwargs)
    ...
    for epoch in range(start_epoch, args.epochs + start_epoch):
        loss = pretrain(train_loader, model, nn.functional.binary_cross_entropy_with_logits, optimizer, len(vcf_reader.label_encoder.classes_), len(vcf_reader.super_label_encoder.classes_), vcf_reader.maf, epoch, args)
...
def pretrain(loader: DataLoader, model: nn.Module, criterion: Callable, optimizer: Optimizer,

#!/bin/bash
# Example Usage: ./isolate_population.sh chr20_kgp_abridged.vcf kgp_samples.csv CEU
VCF_FILE=$1
POPULATION_FILE=$2
POPULATION=$3
FILE_NAME=$(basename $VCF_FILE)
OUTPUT_VCF_FILE="$POPULATION.${FILE_NAME%.gz}"
POPULATION_SAMPLES=""
...
bcftools view -s $POPULATION_SAMPLES $VCF_FILE > $OUTPUT_VCF_FILE
